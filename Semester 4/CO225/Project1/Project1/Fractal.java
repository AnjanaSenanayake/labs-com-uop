...
{ 	
	...
	{ 
		...
		{	
			public void run()
			{
				loopPrint(g,color,400,800,0,400);
			}	
		});
		t2.start();
		...
		{	
			public void run()
			{
				loopPrint(g,color,0,400,400,800);
			}	
		});
		t3.start();
		...
		{	
			public void run()
			{
				loopPrint(g,color,400,800,400,800);
			}	
		});
		t4.start();
	...
	public synchronized void loopPrint(Graphics g,Color color,int xi,int yi,int xj,int yj)
	{
		for(int i=xi;i<=yi;i++)
		{	
			for(int j=xj;j<=yj;j++)
			{	
				...
				{	
					mb.complexValue(i,j);
					bound = mb.boundCheck(mb.getX(),mb.getY(),iterations);
